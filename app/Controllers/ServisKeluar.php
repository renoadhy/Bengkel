...
class ServisKeluar extends BaseController
{
    ...
    public function store()
    {
        ...
        if ($this->request->getPost('tipe_form') == 'add') {
            $cek = $servis_keluar->insert([
                ...
                $detail_servis = new DetailServisModel();
                $last_id = $servis_keluar->last_id();
                for ($count = 0; $count < count($_POST["id_servis"]); $count++) {
                    $cek = $detail_servis->insert([
    ...
    {
        $servis_keluar = new ServisKeluarModel();
        $data['data_servis_keluar'] = $servis_keluar->where('id_servis_keluar', $id)->first();
    ...
    public function detail_penjualan()
    {
        ...
        foreach ($detail_penjualan as $value) {
            $html .= '<tr><td>' . $no++ . ' </td><td>' . $value->nm_servis . ' </td><td>' . "Rp " . number_format($value->harga, 0, ',', '.') . '</td><td>' . "Rp " . number_format($value->total, 0, ',', '.') . '</td></tr>';
        }
        echo $html;
    }
}
